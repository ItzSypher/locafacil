...
export default function MicroAgent() {
  ...
  const panelRef = useDialog(isOpen, () => setIsOpen(false))

  /* O botão flutuante é desmontado enquanto a conversa está aberta, então o
    ...
    <div className="fixed bottom-6 right-6 z-[9990] flex flex-col items-end">
      ...
      {isOpen && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="locagora-titulo"
          tabIndex={-1}
          initial={{ opacity: 0, y: 16, scale: 0.96 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
          className="on-light bg-white w-[300px] sm:w-[360px] rounded-2xl shadow-2xl overflow-hidden mb-4 flex flex-col h-[500px] max-h-[80vh] focus:outline-none"
        >
          <div className="bg-brand-dark p-4 flex items-center gap-4 relative shrink-0">
            ...
            <div className="flex-1 z-10">
              <h2 id="locagora-titulo" className="type-subtitle text-white">Locagora</h2>
              <p className="type-meta text-text-secondary">Assistente especialista</p>
            </div>
            <button type="button" onClick={() => setIsOpen(false)} aria-label="Fechar conversa" className="text-white/60 hover:text-white transition-colors cursor-pointer z-10 w-11 h-11 flex items-center justify-center -mr-2">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          
          <div className="p-4 bg-surface-light flex-1 overflow-y-auto space-y-4">
              ...
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`p-3 rounded-2xl shadow-sm text-sm border ${
                  msg.role === 'user' 
                    ...
                    : 'bg-white text-text-dark rounded-tl-none border-line-soft'
                }`}>
                  {msg.text}
                </div>
              </div>
          ...
          <div className="p-3 bg-white border-t border-line-soft shrink-0">
            <form onSubmit={handleSend} className="flex gap-2 mb-3">
              <input 
                ...
                className="flex-1 bg-surface-light border border-line rounded-full px-4 py-2 text-sm focus:outline-none focus:border-brand-accent"
              />
              <button 
                type="submit" 
                disabled={!input.trim() || loading}
                className="w-11 h-11 bg-brand-accent text-white rounded-full flex items-center justify-center hover:bg-brand-glow disabled:opacity-50 disabled:cursor-default transition-colors shrink-0 cursor-pointer"
              >
            ...
            <a href="https://api.whatsapp.com/send?phone=5521968540185&text=Ol%C3%A1,%20Locagora!%20Vim%20pelo%20site." target="_blank" rel="noopener noreferrer">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full bg-brand-whatsapp hover:bg-brand-whatsapp-hover text-white font-semibold py-3 rounded-xl flex items-center justify-center gap-2 shadow-md transition-all cursor-pointer text-sm"
              >
      ...
      {!isOpen && (
        <motion.button
          ...
          aria-label="Abrir conversa com a Locagora"
